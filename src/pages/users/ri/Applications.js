...
export default function ApplicationsRi() {
    ...
    const fetchApplications = async () => {
        setLoading(true);
        try {
            const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/ri/pending_applications`, {
                headers: {
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (status) => (
                <span style={{
                    color: status === 'RI' ? '#faad14' : status === 'MVRO' ? '#f5222d' : status === 'SVRO' ? '#f5222d': '#52c41a',
    ...
    const submitRemarks = async (values) => {
        try {
            console.log(values);
            await axios.post(
                `${process.env.REACT_APP_BACKEND_URL}/ri/create_report/${selectedApplicationId}`,
    ...
    const handleNavigate = (path) => {
        const basePath = "/ri"; // Define your base path
        if (path === 'dashboard') {
